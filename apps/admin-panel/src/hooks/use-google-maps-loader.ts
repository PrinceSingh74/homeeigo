 ...
 * modern `loading=async` bootstrap + `importLibrary()` pattern. Admin geo
 * console needs maps and geometry (DrawingManager was removed in API v3.65).
 */
declare global {
  interface Window {
    google?: {
...
function injectScript(key: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      ...
      existing.addEventListener("load", () => void onExistingLoad(), { once: true });
      existing.addEventListener("error", () => finish(new Error("maps_load_failed")), { once: true });
      return;
    }
...
export function mapsLoadErrorHint(code: string | null): string {
  ...
  switch (code) {
    ...
    case "maps_timeout":
      return "Maps script timed out. Restart admin dev server after setting the API key.";
