/**
 * Full frontend CPU investigation probe — runtime evidence for all audit surfaces.
 *
 *   node scripts/frontend-cpu-probe.mjs
 */
...
const PAGES = [
  ...
  { id: "digital-twin", path: "/digital-twin", ready: "City Digital Twin" },
  { id: "heatmap", path: "/heatmap", ready: "Demand Heatmap" },
  { id: "observability", path: "/observability", ready: "Observability Dashboard" },
  { id: "ai-systems", path: "/ai", ready: "AI systems" },
  { id: "bookings", path: "/bookings", ready: "Bookings" },
  { id: "support", path: "/support", ready: "Support Operations" },
  { id: "operations", path: "/operations", ready: "Live Operations" },
];
...
async function getCdpMetrics(cdp) {
  try {
    ...
    return {
      ...
      LayoutDuration: map.LayoutDuration ?? null,
      RecalcStyleDuration: map.RecalcStyleDuration ?? null,
      JSHeapUsedSize: map.JSHeapUsedSize ?? null,
    };
  } catch {
    return null;
  }
...
async function measurePage(page, cdp, wsUrls, { id, path, ready }) {
  ...
  const longTasks = await page.evaluate(() => {
    return new Promise((resolve) => {
      ...
      try {
        ...
        setTimeout(() => resolve({ count, totalDurationMs: Math.round(totalDuration) }), 100);
      } catch {
        resolve({ count: -1, totalDurationMs: -1 });
      }
    });
  });
  ...
  const cdpDelta =
    cdpBefore && cdpAfter
      ? {
          task_duration_ms: Math.round((cdpAfter.TaskDuration - cdpBefore.TaskDuration) * 1000),
          script_duration_ms: Math.round((cdpAfter.ScriptDuration - cdpBefore.ScriptDuration) * 1000),
          layout_duration_ms: Math.round((cdpAfter.LayoutDuration - cdpBefore.LayoutDuration) * 1000),
          recalc_style_duration_ms: Math.round(
            (cdpAfter.RecalcStyleDuration - cdpBefore.RecalcStyleDuration) * 1000,
          ),
          js_heap_used_bytes: cdpAfter.JSHeapUsedSize,
        }
      : null;
  ...
  return {
    ...
    longtask_buffered: longTasks.count,
    longtask_duration_ms_buffered: longTasks.totalDurationMs,
    cdp_performance_delta: cdpDelta,
  };
}
...
async function main() {
  ...
  const outPath = join(dirname(fileURLToPath(import.meta.url)), "frontend-cpu-evidence.json");
