...
export function AppleOAuthCallback() {
  ...
  useEffect(() => {
    ...
    async function complete() {
      if (oauthError) {
        setPhase("error");
        setMessage(getAppleOAuthUserMessage(oauthError));
        return;
      }

      const pending = consumeAppleOAuthPending(state);
      ...
      const claim = claimAppleAuthorizationCode(code);
      ...
      const userParam = readAppleOAuthUserParam();
      const result = await runAuthAction(
        () => signInWithApple(code, state, userParam),
        setError,
      );

      if (!result.ok) {
        finishAppleAuthorizationCode(code, false);
        setPhase("error");
        setMessage(result.message);
        return;
      }

      finishAppleAuthorizationCode(code, true);
      setPhase("success");
    ...
    void complete();
  }, [router, showToast, signInWithApple, setError, status]);
