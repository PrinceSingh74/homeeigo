 ...
 * the latest Maps JS API. With `loading=async`, the script's `onload` fires
 * before any classes exist — `google.maps.Map` etc. only appear after the
 * needed libraries are imported, so we await them all before reporting ready.
 */
declare global {
  interface Window {
    google?: {
