/**
 * Stage E Step 14 — Outbox burst, backpressure, drain & processing-latency certification.
 *
 * Inserts controlled event bursts (100 then 500) into authoritative staging DB and
 * monitors DEPLOYED staging outbox processors drain them. Does NOT call processOutboxBatch.
 *
 * Run via Cloud Run Job:
 *   STAGING_EVENTS_CERTIFICATION=1 EVENTS_OUTBOX_ENABLED=true EVENTS_CONSUMERS_ENABLED=true \
 *     bun --env-file=.env.staging run scripts/stage-e-step-14-outbox-drain-cert.ts
 */
import "../src/load-env";
