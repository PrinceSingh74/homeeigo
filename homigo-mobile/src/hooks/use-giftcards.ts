...
export function useGiftCardPurchase() {
  ...
  const purchase = useCallback(
    async (amount: number, opts: { recipientEmail?: string; message?: string }, onDone?: () => void) => {
      if (busy) return;
      setBusy(true);
      try {
        ...
        await openCheckout({
          ...
          onSuccess: async (payload) => {
            try {
              ...
              showToast(res.sentTo ? `Gift card sent to ${res.sentTo}` : `Gift card ready · ${res.code}`);
              onDone?.();
            } catch {
              showToast("Verification failed. It will reflect shortly if charged.");
            } finally {
              setBusy(false);
            }
          },
          onDismiss: () => {
            setBusy(false);
            showToast("Purchase cancelled. No money was charged.");
          },
        });
      } catch {
        setBusy(false);
...
export function useRedeemGiftCard() {
  ...
  return async (code: string, amount?: number) => {
    try {
      ...
      showToast(
        r.remaining > 0
          ? `₹${r.amount.toLocaleString("en-IN")} added · ₹${r.remaining.toLocaleString("en-IN")} left`
          : `₹${r.amount.toLocaleString("en-IN")} added to your wallet`,
      );
      return true;
    } catch {
      showToast("Invalid, expired, or already-used gift card.");
      return false;
    }
  };
}
