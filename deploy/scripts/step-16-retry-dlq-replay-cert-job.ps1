# Step 16 — Retry / DLQ / Operator Replay certification via Cloud Run Job.
...
$REGION = "asia-south1"
$JOB = "homigo-step16-retry-dlq-replay-cert"
...
$SECRET = "STAGING_STEP16_SCRIPT"
$ScriptPath = Join-Path $PSScriptRoot "stage-e-step-16-retry-dlq-replay-cert.ts"

if (-not (Test-Path $ScriptPath)) {
  $ScriptPath = Join-Path (Split-Path $PSScriptRoot -Parent) "..\apps\backend\scripts\stage-e-step-16-retry-dlq-replay-cert.ts"
}
if (-not (Test-Path $ScriptPath)) {
  throw "Step 16 script not found"
}
...
$envFile = Join-Path $env:TEMP "step16-cert-env.yaml"
@"
NODE_ENV: production
APP_ENV: staging
...
$runCmd = "cd /app && bun scripts/stage-e-step-16-retry-dlq-replay-cert.ts"

Write-Host "Deploying $JOB (Step 16 retry/DLQ/replay cert, image $IMAGE)"
...
gcloud run jobs deploy $JOB `
  ...
  --set-secrets="/app/scripts/stage-e-step-16-retry-dlq-replay-cert.ts=${SECRET}:latest,DATABASE_URL=STAGING_DATABASE_URL:latest,REDIS_URL=STAGING_REDIS_URL:latest"
...
if ($execName) {
  gcloud logging read "resource.type=cloud_run_job AND resource.labels.job_name=$JOB AND labels.run.googleapis.com/execution_name=$execName" --project=$PROJECT --limit=500 --format="value(textPayload)" --freshness=60m 2>$null
}
