# Step 13 — Multi-instance outbox concurrency certification via Cloud Run Job.
...
$REGION = "asia-south1"
$JOB = "homigo-step13-multi-instance-cert"
...
$SECRET = "STAGING_STEP13_SCRIPT"
$ScriptPath = Join-Path $PSScriptRoot "stage-e-step-13-multi-instance-cert.ts"

if (-not (Test-Path $ScriptPath)) {
  $ScriptPath = Join-Path (Split-Path $PSScriptRoot -Parent) "..\apps\backend\scripts\stage-e-step-13-multi-instance-cert.ts"
}
if (-not (Test-Path $ScriptPath)) {
  throw "Step 13 script not found"
}
...
$envFile = Join-Path $env:TEMP "step13-cert-env.yaml"
@"
NODE_ENV: production
APP_ENV: staging
...
$runCmd = "cd /app && bun /secrets/step13-cert.ts"

Write-Host "Deploying $JOB (Step 13 multi-instance cert, image $IMAGE)"
...
gcloud run jobs deploy $JOB `
  ...
  --set-secrets="/secrets/step13-cert.ts=${SECRET}:latest,DATABASE_URL=STAGING_DATABASE_URL:latest,REDIS_URL=STAGING_REDIS_URL:latest"
...
if ($execName) {
  gcloud logging read "resource.type=cloud_run_job AND resource.labels.job_name=$JOB AND labels.run.googleapis.com/execution_name=$execName" --project=$PROJECT --limit=300 --format="value(textPayload)" --freshness=60m 2>$null
}
