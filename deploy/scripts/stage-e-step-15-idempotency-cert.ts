...
import { bootstrapEventConsumers } from "/app/src/events/consumers/index.ts";
...
import { matchConsumers } from "/app/src/events/core/consumer-registry.ts";
...
async function main() {
  ...
  const readyAuthGated = ready.status === 401;
  gate(
    "pre.ready",
    ready.status === 200 || readyAuthGated ? "PASS" : "FAIL",
    readyAuthGated ? `HTTP ${ready.status} (EXPECTED_AUTH)` : `HTTP ${ready.status}`,
  );
  ...
  const evidence = {
    ...
    idempotencyArchitecture: {
      ...
      sourcePaths: [
        "apps/backend/src/events/core/event-bus.ts",
        "apps/backend/src/events/core/idempotency.ts",
      ],
    },
    testEvent: {
      STEP15_RUN_ID,
    ...
    NON_CRITICAL_WARNINGS: readyAuthGated ? 1 : 0,
    warnings: readyAuthGated
      ? [{ id: "ready-auth-gated", severity: "NON_CRITICAL", detail: "/ready returns 401 without OPS token — same as Steps 10–14" }]
      : [],
  };
