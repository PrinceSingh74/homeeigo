# Step 12 — Payment flow, financial atomicity & event certification via Cloud Run Job.
...
$REGION = "asia-south1"
$JOB = "homigo-step12-payment-cert"
...
$ScriptPath = Join-Path $PSScriptRoot "stage-d-step-12-payment-cert.ts"

if (-not (Test-Path $ScriptPath)) {
  throw "Step 12 script not found: $ScriptPath"
}

# Script exceeds Cloud Run 32KiB env limit — store in Secret Manager and mount as file.
$secretExists = $false
try {
...
$envFile = Join-Path $env:TEMP "step12-cert-env.yaml"
@"
NODE_ENV: production
APP_ENV: staging
...
Write-Host "Deploying $JOB (Step 12 payment cert, image $IMAGE)"
...
if ($execName) {
  gcloud logging read "resource.type=cloud_run_job AND resource.labels.job_name=$JOB AND labels.run.googleapis.com/execution_name=$execName" --project=$PROJECT --limit=200 --format="value(textPayload)" --freshness=30m 2>$null
}
